...
get_describe_info() {
    ...
    mkdir ${des_dir}
    for attr in gateway serviceentry destinationrule virtualservice statefulsets internalCertificates crd deployments services replicasets endpoints daemonsets persistentvolumeclaims configmap pods nodes jobs persistentvolumes rolebindings roles secrets serviceaccounts storageclasses ingresses httpproxy networkpolicies
        do
...
function pg_dump_log(){
  dumps=${log_base_path}/logs/dump
  mkdir $dumps
  POD_COUNT=$(kubectl get po -n ${namespace} | grep "eric-dmm-common-db" | wc -l)
  POD_NAME=$(kubectl get pod -n ${namespace}  | grep "eric-dmm-common-db" | awk {'print $1'} | tr '\n' ' ')
  POD_NAME_ARR=($POD_NAME)
  for (( i=0; i<${#POD_NAME_ARR[@]}; i++ ));
  do
  kubectl --namespace $namespace exec ${POD_NAME_ARR[i]}  -- pg_dump -U postgres catalog --inserts --no-owner > $dumps/${POD_NAME_ARR[i]}-pg-dump.sql

...
docker build -t armdocker.rnd.ericsson.se/proj-edca-dev/k6-adc-testsuite:1.78 --build-arg hostname=${ADC_HOSTNAME} --no-cache -f ./deployment/Dockerfile .
docker push armdocker.rnd.ericsson.se/proj-edca-dev/k6-adc-testsuite:1.78
...
kubectl --namespace ${NAMESPACE} --kubeconfig ${KUBECONFIG} apply -f ./deployment/charts/network-policy/eric-adc-data-collection-controller.yaml;

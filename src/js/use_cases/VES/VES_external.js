...
//Method to Verify posting Single/Batch Event objects to event Listener
export default function() {
    group('Virtual Event Streaming (VES) Collector Flow', function() {
        console.log("============================================VES-Collector Logs=========================================")
        ...
        //             'Status check 400': (res) => (res.status === 400 && res.body.includes("JSON does not conform to schema") === true)
        //         });
        //     })
        //     console.log("====================================================================================================")
        // }
        ...
        //     check(res_data, {
        //         'Status check 400': (res) => (res.status === 400 && res.body.includes("Incorrect request api version") === true)
        //     });
        // })
        ...
        //     check(res_data2, {
        //         'Status check 400': (res) => (res.status === 400 && res.body.includes("Incorrect request Content-Type") === true)
        //     });
        // })
        ...
        //     check(res_wrong, {
        //         'status check 400': (res) => (res.status === 400 && res.body.includes("Invalid json content!") === true)
        //     });
        // })
        ...
        for (let i = 0; i < get_res.length; i++) {
            group("Verify Welcome msg to VESCollector" + ".", function() {
                console.log("Status : " + get_res[i].status + " Response :" + get_res[i].body);
                check(get_res[i], {
                    'Status check 200': (res) => (res.status === 200 && res.body.includes("Welcome to VESCollector") === true)
                });
            })
        }
    })
    console.log("===================================END 0f VES-COLLECTOR LOGS================================")
}
